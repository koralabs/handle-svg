...
export default class HandleSvg {
    ...
    buildTextRibbon = () => {
        const getGradientStops = (colors: string[]) => {
            ...
            for (let i = 0; i < colors.length; i++) {
                stops.push(
                    `<stop offset="${(i / (colors.length - 1)) * 100}%" style="stop-color:${
                        colors[i]
                    };stop-opacity:1" />`
                );
            }
        ...
        if (textRibbonColorsEnabled && textRibbonColors && textRibbonColors.length > 0) {
            ...
            if (
                ...
                <svg>
                    ...
                    <rect x="0" y="${
                        size / 2 - ratio * 50
                    }" width="${size}" height="${height}" style="fill: url(#grad1)" />
                </svg>
                `;
            }
    ...
    buildQRCode = () => {
        ...
        if (qrEnabled) {
            return `<svg id="qr_code_${handle}" x="${size - ratio * 125}" y="${size - ratio * 125}"></svg>`;
        }

        return '';
    };

    renderSocialIcon({ social, heightPercentage }: { social: string; heightPercentage: number }) {
        const x = 15;
        const { size, ratio } = this._params;
